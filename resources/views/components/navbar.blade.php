        ...
        <div class="flex items-center space-x-3 rtl:space-x-reverse md:order-2 md:space-x-0">
                        ...
                        <ul class="py-2" aria-labelledby="user-menu-button">
                            <li class="mb-2">
                                <a href="{{ route('dashboard') }}"
                                    class="{{ Route::currentRouteName() == 'dashboard' ? 'bg-[#f00c93] text-white' : '' }} block rounded px-4 py-2 text-sm text-gray-700 hover:bg-[#f00c93] hover:text-white">Dashboard</a>
                            </li>
                            <li>
                                <form action="/logout" method="POST">
                                    @csrf
                                    <button type="submit"
                                        class="block w-full rounded px-4 py-2 text-start text-sm text-gray-700 hover:bg-[#f00c93] hover:text-white">
                                        Sign out
                                    </button>
                                </form>
                            </li>
            ...
            @guest
                ...
                <div class="flex justify-center">
                    <a href="{{ route('login') }}"
                        class="text-md md:hidden w-full justify-center rounded-md bg-[#f00c93] px-6 py-2 font-bold leading-6 text-white shadow-sm hover:bg-[#ed6fb7] focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 md:flex">{{ $settings['navbar_login_button'] }}</a>
                </div>
